...
(function (context, dimple) {
    ...
    if (typeof exports === "object") {
        ...
        if (typeof define === "function" && define.amd) {
            ...
            define(["d3"], function (d3) {
                // publish ple to the global namespace for backwards compatibility
    ...
    dimple.axis = function (chart, position, categoryFields, measure, timeField, autoRotateLabel) {;
        ...
        this.basicTooltip = null;

        this.detailTooltip = null;
        this.detailTooltipdata = null;
        this.detailTip = {"bar":false,"point":false,"shape":false};
        this.xField;
        this.tooltipFields= [];
        this.tooltipSeries=[];
        this.hasTimeAxis = false;
        ...
        this._getTooltipText = function (rows, d) {
            if (this._hasTimeField()) {
                if (d[this.position + "Field"][0]) {
                    var newVal = this.categLabels[this._getFormat()(d[this.position + "Field"][0])]!=undefined?this.categLabels[this._getFormat()(d[this.position + "Field"][0])]:this._getFormat()(d[this.position + "Field"][0])
                    rows.push(this.timeField + ": " + newVal);
                }
                ...
                case "x":
                    ...
                    rows.push(this.measure + ": " + d3.round(d.width));
                    break;
                case "y":
                    //rows.push(this.measure + ": " + this._getFormat()(d.height));
                    rows.push(this.measure + ": " + d3.round(d.height));
                    break;
                case "p":
                ...
                default:
                    ...
                    rows.push(this.measure + ": " + d3.round(d[this.position + "Value"]));
                    break;
                }
            }
        };
                ...
                if (!this.hidden) {
                    ...
                    case 0:
                        this._draw = d3.svg.axis()
                            .orient("bottom")
                            .scale(this._scale)
                            if(!this._hasTimeField() && this.measure==null) 
                            {
            ...
            this.axes.forEach(function (axis) {
                ...
                if (axis.gridlineShapes !== null) {
                    handleTrans(axis.gridlineShapes.selectAll("line"))
                        ...
                        .attr("class", appendClass("gridlineConfig"))
                        .call(function() {
                            if (!chart.noFormats) {
                                //this.style("fill", "none")
...
// **** Modification Ends ****
        ...
        this._draw = function () {
            ...
            if(this.InitlegendArray == null)
                this.InitlegendArray = this._getEntries();

            var legendArray = this.InitlegendArray,
                maxWidth = 0,
                maxHeight = 0,
                runningX = 0,
                runningY = 0,
...
// **** Modification ends ****;
    ...
    dimple.storyboard = function (chart, categoryFields) {
        ...
        // The category values
        this._categories = [];
        // The csategory values when the last cache happened
    ...
    dimple.plot.bar = {
        ...
        draw: function (chart, series, duration) {
            ...
            updated = chart._handleTransition(theseShapes, duration, chart, series)
                ...
                .call(function () {
                    if (!chart.noFormats) {
                        this.style("fill", function (d) { return dimple._helpers.fill(d, chart, series); })
                            .style("stroke", function (d) { return dimple._helpers.stroke(d, chart, series); });
                    }
                });
    ...
    dimple.plot.bubble = {
        ...
        draw: function (chart, series, duration) {
            ...
            theseShapes
                ...
                .call(function () {
                    if (!chart.noFormats) {
                        this.attr("opacity", function (d) { return dimple._helpers.r(d, chart, series)==1?0:dimple._helpers.opacity(d, chart, series); })
                            .style("fill", function (d) { return dimple._helpers.fill(d, chart, series); })
                            .style("stroke", function (d) { return dimple._helpers.stroke(d, chart, series); });
                    }
                });
...
//series.shape="polygon"
            // Add
            theseShapes
                .enter()
                .append(series.shape=="polygon"?"polygon":"path")
               // .attr("d", function(s){return s.zValue==0?"":"M 30 30 L 55 5 L 80 30 L 55 55 Z"})
                .attr("d", d3.svg.symbol()
                    .size( function(d) { return d.zValue})
                    .type( function(d) { return series.shape }))
                .attr("points", function(d){
                    return (d.zValue/2)+',0 '+d.zValue+','+(d.zValue/2)+' '+(d.zValue/2)+','+d.zValue+' 0,'+(d.zValue/2);
                    //    return (shape(d.zValue))();
               })
                .attr("id", function (d) { return dimple._createClass([d.key]); })
                .attr("class", function (d) {
                    var c = [];
...
//                .attr("r", 0)
                ...
                .on("mouseover", function (e) { dimple._showShapeTooltip(e, this, chart, series); })
                .on("mouseleave", function (e) { dimple._removeTooltip(e, this, chart, series); })
                .call(function () {
                    if (!chart.noFormats) {
                        this.attr("class","shapeConfig")
    ...
    dimple._drawMarkers = function (lineDataRow, chart, series, duration, className, useGradient, enterEventHandler, leaveEventHandler, lineShape) {
        ...
        shapes
            ...
            .attr("opacity", (series.lineMarkers ? lineDataRow.color.opacity : 0))
            .call(function () {
                if (!chart.noFormats) {
    ...
    dimple._removeTooltip = function (e, shape, chart, series) {
        ...
        if(chart.detailTooltip != null){
            chart.detailTooltip.hide();
        }
        if(chart.basicTooltip != null){
            chart.basicTooltip.hide();
        }
        //if(dimple.tooltip == undefined) return;
        //dimple.tooltip.hide();
    };
    ...
    dimple._refineToolTips = function(lclData,chart,xValue)
    {
        //remove unwanted series
        if(chart.tooltipSeries.length > 0)
        {
            lclData.forEach(function(d,i){
                if(chart.tooltipSeries.indexOf(d['X']) == -1)
                    d['filter'] = true;
            })
        }
        //add additional fields
        var addonData,node;
        for(var t=0;t<chart.tooltipFields.length;t++)
        {
            addonData = this.filterData(chart.data,chart.xField,xValue)
            node = {}
            node['A'] = xValue
            node['X'] = chart.tooltipFields[t]
            node['Y'] = addonData[0][chart.tooltipFields[t]]
            node['Color'] = "white"
            node['highlight'] = false;
            node['header'] = true;
            node['filter'] = false;

            lclData.splice(0, 0, node);
        }
        // /lclData.push(0,node)

    }

    dimple._showCustomTooltip = function(e, shape, chart, series, chartType, tx, ty)
    {
        ...
        if(chart.detailTooltip != null)
        {
            if(chart.detailTooltipdata.length == 0) return;

            var returnFormat = d3.time.format("%m/%d/%Y");
            var xValue=[];

            if(chart.hasTimeAxis)
                xValue.push(returnFormat(e.x))
            else
                xValue.push(e.x);

            var lclData = this.filterData(chart.detailTooltipdata,"A",xValue)

            //if(chart.detailTip[chartType])
            lclData.forEach(function(d){
                d['highlight'] = e.aggField==d['X']?true:false;
                d['header'] = false;
                d['filter'] = false;
            })

            lclData = chart.detailTip[chartType]?lclData:this.filterData(lclData,"X",e.aggField)

            this._refineToolTips(lclData,chart,xValue);

            lclData = this.filterData(lclData,"filter",['false'])
            if(lclData.length==1)
                lclData[0]["highlight"] = false;
        ...
        else
        {
            ...
            var tooltipdata=series.getTooltipText(e),tooltext=[];
           
            Object.keys(tooltipdata).forEach(function(key) {
                if(chart.tooltipFields.length>0) 
                {
                    if(chart.tooltipFields.indexOf(tooltipdata[key].split(': ')[0]) > -1)
                        tooltext[tooltipdata[key].split(': ')[0]] = tooltipdata[key].split(': ')[1];
                }
                else
                        tooltext[tooltipdata[key].split(': ')[0]] = tooltipdata[key].split(': ')[1];
    ...
    dimple._showBarTooltip = function (e, shape, chart, series) {
        ...
        chart._tooltipGroup.select("g").remove();

        this._showCustomTooltip(e, shape, chart, series, "bar",transformer.x,transformer.y);
    ...
    dimple._showShapeTooltip = function (e, shape, chart, series) {
        ...
        // Collect some facts about the highlighted bubble
            selectedShape = d3.select(shape),
            transform = d3.transform(selectedShape.attr("transform")), //selectedShape[0][0].getAttribute("transform").replace("translate(","").replace(")",""),
            x = transform.translate[0],//split(",")[0],
       ...
       /* if (dropDest.y !== null) {
        ...
        //translateX = parseFloat(transform.split(",")[0])+20;
        //translateY = transform.split(",")[1];
        
        chart._tooltipGroup.select("g").remove();

        this._showCustomTooltip(e, shape, chart, series, "shape",parseFloat(x),parseFloat(y));
    };
    ...
    dimple._showPointTooltip = function (e, shape, chart, series) {
        ...
        this._showCustomTooltip(e, shape, chart, series, "point",translateX,translateY);
